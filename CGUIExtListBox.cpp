...
//! constructor
CGUIExtListBox::CGUIExtListBox(IGUIEnvironment* environment, IGUIElement* parent, 
...
s32 CGUIExtListBox::getItemCount()
{
	return Items.size();
}
...
const wchar_t* CGUIExtListBox::getListItem(s32 id)
{
...
s32 CGUIExtListBox::addItem(const wchar_t* text)
{
	ListItem i;
	i.text = text;
// --------------------------------------------------------
...
void CGUIExtListBox::recalculateItemHeight()
{
	IGUISkin* skin = Environment->getSkin();
...
s32 CGUIExtListBox::getSelected()
{
	return Selected;
}
...
void CGUIExtListBox::setSelected(s32 id)
{
	if (id<0 || id>((s32)Items.size())-1)
		Selected = -1;
	else
...
bool CGUIExtListBox::OnEvent(SEvent event)
{
...
void CGUIExtListBox::selectNew(s32 ypos, bool onlyHover)
{
...
// --------------------------------------------------------
				  if(i == Selected){
   					IconFont->draw(Items[i].icon.c_str(), textRect, skin->getColor(EGDC_HIGH_LIGHT_TEXT), false, true, &clientClip);
    			}else{
   					IconFont->draw(Items[i].icon.c_str(), textRect, Items[i].color, false, true, &clientClip);
    			}
// --------------------------------------------------------
        }
...
// --------------------------------------------------------
			  if(i == Selected){
		  		Font->draw(Items[i].text.c_str(), textRect, skin->getColor(EGDC_HIGH_LIGHT_TEXT), false, true, &clientClip);
			   	...
			   	Font->draw(Items[i].text.c_str(), textRect, Items[i].color, false, true, &clientClip);
			  }
// --------------------------------------------------------
...
s32 CGUIExtListBox::addItem(const wchar_t* text, const wchar_t* icon)
{
	ListItem i;
	i.text = text;
	i.icon = icon;
...
void CGUIExtListBox::setIconFont(IGUIFont* font)
{
	if (IconFont)
		IconFont->drop();
...
// --------------------------------------------------------
void CGUIExtListBox::setListTopItem(s32 index){
	IGUISkin* skin = Environment->getSkin();
	if(Font != skin->getFont()){
		if(Font) Font->drop();
		Font = skin->getFont();
  ...
  ScrollBar->setPos(ItemHeight * index);
}

s32 CGUIExtListBox::addItem(const wchar_t* text, video::SColor color){
	ListItem i;
	i.text = text;
  i.color = color;
	Items.push_back(i);
...
void CGUIExtListBox::setItemColor(s32 index, video::SColor color){
  if((index < Items.size()) && (index >= 0)){
    Items[index].color = color;
  }
}

s32 CGUIExtListBox::addItem(const wchar_t* text, video::SColor color, const wchar_t* icon){
	ListItem i;
	i.text = text;
	i.icon = icon;
  i.color = color;
